  ...
  try {
    ...
    if (data.success) {
      ...
      contracts = data.contracts.filter(
        (contract) =>
          contract.clientEmail === email && contract.reviewed === false
      );
...
const writeToReview = async (review, contractId) => {
  try {
    ...
    if (data.success) {
      // Update the 'reviewed' field to true in the backend
...
const markContractReviewed = async (contractId) => {
  try {
    const response = await fetch("/update-contract", { // Use the correct endpoint
      method: "POST", // Update this to POST as per your backend
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ contractId, action: "markReviewed" }), // Specify the action
    });
