...
const LoginClient = "Dylan Cheballah";//This will be used when a technician logs in and used to filted only the contracts assigned to that technician
...
app.get("/technicianmobile", async (req, res) => {
  try {
    ...
    const upcomingHTML = upcomingContracts.map(el => 
      replaceTechDashboardTemplate(temptechmobileUpcoming, el)).join("");

    const scheduledHTML = scheduledContracts.map(el => 
      replaceTechDashboardTemplate(temptechmobileSchedule, el)).join("");
